...
function chooseGoods(){
	...
	var htmls = "<tr style='text-align:center'>"
				...
				+"<td>"+ goodsCode +"</td>"
				+"<td>"+ unit +"</td>"
				+"<td><input class='form-control' name='goodsNum'  gn='"+goodsName+"' gc='"+goodsCode+"' unit='"+unit+"' /></td>"
...
function saveGoods(obj){
	...
	if(flag){
		var detail = "";
		$("#table" + type + " input[name='goodsNum']").each(function(){
			detail += $(this).attr("gc") + "," + $(this).attr("gn") + "," + $(this).val() + "," + $(this).attr("unit") + ",";
		});
...
function showInventory(){
	...
	var flag4 = true;
	var flag5 = true;
	for(var i = 0;i < arrs.length;i = i + 4){
...
function checkGoods(details,arrs){
	for(var i = 0;i < details.length - 1;i = i + 4){
		var flag = true;
		for(var j = 0;j < arrs.length;j = j + 4){
...
function showOrderDetail(obj){
	...
	var htmls = "";
	$("#tbody6").html("");
	for(var i = 0;i < goodsArr.length - 1;i = i + 4){
//		var detailArr = goodsArr[i].split(",");
...
function checkNum(obj){
	...
	var outFlag = false;
	for	(var i = 0;i < detailArr.length - 1;i=i+4) {
		//填的跟原来的数字不一样
...
function showAllDetail(){
	...
	var htmls = "";
	var num = 1;
	for(var i = 0;i < goodsDetail.length;i = i + 4){
...
function seed(){
	...
	for (var i = 0;i < goodsDetail.length;i = i + 4) {
		...
		if (goodsDetail[i] != '6920459905395') {//6920459905395:康师傅水
			...
			$("#num3").val(num3);
			for(var i = 1;i < 4;i++){
				var ind = priorityList.indexOf(""+i);//找出各个优先级对应的订单，为index + 1
...
function checkInventory(id,index){
	var flag = true;
//	var index = $(this).attr("in");//id命名的顺序
	var priorityList = get_address("priorityList").split(",");//优先级顺序
...
//				$("#dealBtn").show();
				...
				var htmls = "<div align='center' class='modal-footer'><button type='button' onclick='showOrderList("+obj+")' class='btn btn-primary'>确定</button></div>";
				$("#btnDiv").empty();
				$("#btnDiv").html(htmls);
				...
				if (orderDetail.length == 5) {
					...
					var n = 0;
					for (var j = ind;j < orderDetail.length - 4;j++) {
						orderDetail[j] = orderDetail[j+4];
						n = j;
					}
				...
				$("#btnDiv").empty();
				$("#btnDiv").html(htmls);
				$("#dialogSpan").html("由于当前订单："+orderName+" 中的康师傅水库存不足，并且配送措施选择：公司政策不允许分批出货，删除订单中不足额部分商品。所有需要先删除库存不足的产品，再生成拣货单。"); 
				$("#alertDiv").modal("toggle");
				$("#dealBtn").show();
...
function getOrderGoodsList(orderDetail){
	if (orderDetail.length > 4) {
		...
		var htmls = "";
		for (var i = 0;i < orderDetail.length - 1;i = i + 4) {
			if (orderDetail[i] != ""){

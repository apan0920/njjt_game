...
$(function(){
	...
	var row1 = [
		' <span draggable="true" step="9"><i></i><em>码头安排货箱装船</em></span> ',
		' <span draggable="true" step="4"><i></i><em>托运人提箱</em></span> ',
		' <span draggable="true" step="1"><i></i><em>托运人向承运人订舱</em></span> ',
		' <span draggable="true" step="8"><i></i><em>托运人安排货箱集港</em></span> '
	];
	var row2 = [
		' <span draggable="true" step="2"><i></i><em>承运人接受托运申请</em></span> ',
		' <span draggable="true" step="3"><i></i><em>承运人放箱</em></span> ',
		' <span draggable="true" step="7"><i></i><em>托运人报关</em></span> ',
		' <span draggable="true" step="11"><i></i><em>承运人向托运人发放提单</em></span> '
	];
	var row3 = [
		' <span draggable="true" step="5"><i></i><em>托运人装箱</em></span> ',
		' <span draggable="true" step="10"><i></i><em>码头向承运人回馈装船信息</em></span> ',
		' <span draggable="true" step="6"><i></i><em>托运人向承运人发送装箱信息</em></span> '
	];

	$('#sourceBox').html('');
	$.each(arrRand(row1),function(i,item){
		$('#sourceBox').append(item);
	});
	$.each(arrRand(row2),function(i,item){
		$('#sourceBox').append(item);
	});
	$.each(arrRand(row3),function(i,item){
		$('#sourceBox').append(item);
	});
	...
	$('.btn_1').click(function(){
		...
		if(error.length<=0){
			showAlert('恭喜您！全部答对。','end',function(obj){
				...
				$('.export_content').removeClass('bounceInDown').addClass('delay_05').addClass('bounceOutUp');
				setTimeout(function(){
					obj.remove();
